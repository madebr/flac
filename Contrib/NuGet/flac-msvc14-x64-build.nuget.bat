...
cd %BUILDTREE%\deps
SET OGGVER=1.3.2.7782
nuget install ogg-msvc14-%tbs_arch% -Version %OGGVER%
...
rd /s /q %BUILDTREE%\flac
mkdir %BUILDTREE%\flac
cd %BUILDTREE%\flac
...
-DOGG_LIBRARIES=%BUILDTREE%\deps\ogg-msvc14-%tbs_arch%.%OGGVER%\build\native\lib_release\ogg.lib ^
-DOGG_INCLUDE_DIRS=%BUILDTREE%\deps\ogg-msvc14-%tbs_arch%.%OGGVER%\build\native\include ^
...
copy %INITDIR%\flac-msvc14-%tbs_arch%.targets %BINDIR%\flac-msvc14-%tbs_arch%.targets
cd %BUILDTREE%
nuget pack %INITDIR%\flac-msvc14-%tbs_arch%.nuspec
cd %INITDIR%
REM --- exit ----
GOTO:eof
